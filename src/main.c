...
int main( int argc, char *argv[] )
{
  ...
  /* Parse args */
    ...
    while ( (c = getopt(argc, argv, "h:P:d:u:p:k:C:w:c:r:l:i:f:t:m:o:S:0:1:2:3:4:")) != -1) {
        switch (c) {
        case 'h':
        ...
        case '4':
            ...
            break;
        case '?':
    	    printf("Usage: tpcc_start -h server_host -P port -d database_name -u mysql_user -p mysql_password -k key -C cert -w warehouses -c connections -r warmup_time -l running_time -i report_interval -f report_file -t trx_file\n");
            exit(0);
        default:
...
int thread_main (thread_arg* arg)
{
  ...
  if(is_local==1){
    ...
    mysql_ssl_set(ctx[t_num], db_key, db_cert, NULL, NULL, NULL);
